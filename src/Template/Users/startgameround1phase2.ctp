...
<body>
    <input type="hidden" id="check_status_r1p2" value="">
    <input type="hidden" id="waiting_screenID" name="waiting_screen" value="12">

    <input type="hidden" id="game_status_r1p2" value="">
        ...
        <div class="row">
            ...
            <div class="col-sm-6">
                ...
                <div class="row">
                    <div class="col-sm-12" style="text-align:center;">
                        <span style="color: #009B91;font-weight: bold;padding: 0.375rem 0.75rem;width: -webkit-fill-available;font-size: larger;">
                            Proceed with Round 1 - Execution Phase
                        </span>
                    </div>
                </div><br>
                ...
                <div class="row">
                    ...
                    <div class="col-sm-6">
                        <?php
                        if ($totalgamecode == 4 && $total_r1p2_waitingcnt == 4) {
                            echo $this->Html->link(
                                'Start Game',
                                ['controller' => 'Users', 'action' => 'waiting2'], //round1phase2
    ...
    <script>
        ...
        $(document).ready(function() {
            checkr1p2status();
            checkr1p2gamestatuscount();
            var interval1 = setInterval(function() {
                var playerStatus = $('#check_status_r1p2').val();
                var gameStatusCount = $('#game_status_r1p2').val();
        ...
        function checkr1p2gamestatuscount() {
            return $.ajax({
                url: "round1p2started",
                type: "GET",
                success: function(data) {
                    ...
                    $('#game_status_r1p2').val(game_status);
                }
            })
        }

        function checkr1p2status() {
            return $.ajax({
                url: "getr1p2playerStatus",
                type: "GET",
                success: function(data) {
                    var p_status = JSON.parse(data);
                    console.log(p_status);
                    $('#check_status_r1p2').val(p_status);
                }
            })
        }
    </script>
