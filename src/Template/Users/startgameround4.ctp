...
<body>
    <input type="hidden" id="check_status_r2p2" value="">
    <input type="hidden" id="game_status_r2p2" value="">
    <input type="hidden" id="game_status" name="game_status" value="Started">

    <!-- check user should be on waiting screen -->
    <input type="hidden" id="waiting_screenID" name="waiting_screen" value="22">
        ...
        <div class="row">
            ...
            <div class="col-sm-6">
                ...
                <div class="row">
                 ...
                 Proceed with Round 2 - Execution Phase   
                    </span>
                    </div>
                    </div><br>
                <div class="row">
                ...
                <div class="row">
                    ...
                    <div class="col-sm-6">
                        <?php
                        if ($totalgamecode == 4 && $total_r2p2_waitingcnt == 4) {
                            echo $this->Html->link(
                                'Start Game',
                                ['controller' => 'Users', 'action' => 'waiting4'],
    ...
    <script>
        $(document).ready(function() {
            checkr1p1status();
            checkr2p2gamestatuscount();
            var interval1 = setInterval(function() {
                var playerStatus = $('#check_status_r2p2').val();
                var gameStatusCount = $('#game_status_r2p2').val();
        ...
        function checkr1p1status() {
            return $.ajax({
                url: "getr2p2playerStatus",
                type: "GET",
                success: function(data) {
                    var p_status = JSON.parse(data);
                    console.log(p_status);
                    $('#check_status_r2p2').val(p_status);
                }
            })
        }

        function checkr2p2gamestatuscount() {
            return $.ajax({
                url: "round2p2started",
                type: "GET",
                success: function(data) {
                    ...
                    $('#game_status_r2p2').val(game_status);
                }
            })
        }
    </script>
