...
<section id="admin">
  ...
  <div class="sidebar" style="overflow-x: hidden;">
    ...
    <?php echo $this->Html->link(
    'Online Games',
            ['controller' => 'OnlineGames', 'action' => 'index'],['class' => 'nav-link active']
...
<!--    <input id="filter" type="text" class="form-control" placeholder="Search Here ...">-->
    ...
    <?= $this->Form->create($onlineGame) ?>
                ...
                <?php
                echo "<p>" . $this->Form->control('game_name',['class' => 'form-control', 'style' => "width: 50%;"]). "</p>";
                echo "<p>" . $this->Form->control('game_players_allowed', ['class' => 'form-control', 'style' => "width: 50%;"])."</p>";
                echo "<p>" . $this->Form->control('survey_id', ['options' => $surveys, 'empty' => true,'class' => 'form-control', 'style' => "width: 50%;"])."</p>";
                echo "<p>" . $this->Form->control('shape_group_id', ['options' => $shapeGroups, 'empty' => true, 'class' => 'form-control', 'style' => "width: 50%;"])."</p>";
                echo "<p>" . $this->Form->control('status',['class' => 'form-control', 'style' => "width: 50%;"])."</p>";
                echo "<p>" . $this->Form->control('start_time',['class' => 'form-control', 'style' => "width: 50%;"])."</p>";
                ?>
                <br>
...
<!-- <nav class="large-3 medium-4 columns" id="actions-sidebar">
    <ul class="side-nav">
        ...
        <li><?= $this->Form->postLink(
                __('Delete'),
                ['action' => 'delete', $onlineGame->id],
                ['confirm' => __('Are you sure you want to delete # {0}?', $onlineGame->id)]
            )
        ?></li>
...
<div class="onlineGames form large-9 medium-8 columns content">
    <?= $this->Form->create($onlineGame) ?>
    <fieldset>
        <legend><?= __('Edit Online Game') ?></legend>
        <?php
